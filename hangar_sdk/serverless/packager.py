...
from hangar_sdk.serverless.utils import UI
...
the hangar vendor folder.

Your deployment will continue but may not work correctly
if missing dependencies are not present. For more information:
https://github.com/hangar-labs/Hangar
...
class BaseLambdaDeploymentPackager(object):
    ...
    def _add_py_deps(
        ...
        for root, dirnames, filenames in self._osutils.walk(deps_dir):
            if root == deps_dir and "hangar" in dirnames:
                # Don't include any hangar deps.  We cherry pick
    ...
    def inject_latest_app(
        ...
        """Inject latest version of hangar app into a zip package.

        This method takes a pre-created deployment package and injects
        in the latest hangar app code.  This is useful in the case where
        you have no new package deps but have updated your hangar app code.
        ...
        :param project_dir: Path to hangar project dir.
        ...
        with self._osutils.open_zip(deployment_package_filename, "r") as inzip:
            with self._osutils.open_zip(
                ...
                # Then at the end, add back the app.py, hangarlib,
                # and runtime files.
    ...
    def _needs_latest_version(self, filename: str) -> bool:
        return filename == "app.py" or filename.startswith(("hangarlib/", "hangar/"))
...
class PipRunner(object):
    """Wrapper around pip calls used by hangar."""
